...
However, two endpoints (for listing all certificates issued by this authority
and listing all issuers in this mount) are authenticated, though not generally
...
Note that the response differs between the legacy `/pki/cert/cert` path and
